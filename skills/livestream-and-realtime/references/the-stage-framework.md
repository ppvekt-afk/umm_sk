# The STAGE framework — plan, promote, run + repurpose a live

Live is native — the human runs it in-app. STAGE makes the **schedulable parts** (promo + repurpose) and the
**human parts** (run-of-show + live engagement) explicit. The agent **plans + schedules promo + advises the
live playbook + plans repurpose**; **WoopSocial schedules the promo + publishes the clips**; **the broadcast +
...
- **Hook + pinned topic** → **5–7 pre-submitted Qs** (Stories Q&A sticker) → engagement → **CTA.** Invite a
  **guest/collab** (appears in both Stories bars). **Tech check:** lighting, audio, **≥10 Mbps**, clean lens.

## A — Announce + promote before
- **Schedule the Live** (follower notification + reminder) + **teaser/countdown/BTS** — **scheduled via
...
## E — Extract + repurpose
- Save the recording → **clips** (→ `captions-and-clipping`) → **scheduled via WoopSocial** + replay; reuse
  across time/space (→ `content-recycling`/`cross-platform-repurposing`). **One live → a multi-phase
  campaign.**

## The live plan a request should fill
```
