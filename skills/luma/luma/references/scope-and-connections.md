...
- **ai-video** = the model-agnostic video-generation router/craft (read it first; this is the Luma-specific
  lane).
- **veo-3 / kling / runway** = sibling video tools — route sound-native generation to Veo/Kling (Luma has no
  native audio); strengths shift per release, test on your brief.
