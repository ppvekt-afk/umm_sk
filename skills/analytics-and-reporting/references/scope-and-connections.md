# Scope, the keystone role + connections

analytics-and-reporting is the **measurement keystone** of the library — it reads native platform data,
turns it into goal-mapped insight, and closes the loop into the next cycle. It's where **every "read
native analytics to pick winners" instruction across the other skills resolves.**

## Honest scope (never violate)
- **WoopSocial has NO analytics surface.** The source of truth is each platform's **native dashboard**
  (+ **GA4/UTM** for conversions). The **human pulls/pastes** the numbers; **the agent interprets and
  reports.**
- **Never fabricate, estimate-as-fact, inflate, or cherry-pick** a metric. If a number wasn't provided,
  **flag the gap** — don't invent it. Cite the **native source** for each figure.
- **Read honestly:** correlation ≠ causation; **single posts/short windows are noisy**; **attribution
  undercounts ~30–50%** — report directional signals, not false precision.
- **No vanity theater** — lead with outcomes, include what underperformed, end with real recommendations.
- **Privacy:** aggregate, not creepy individual tracking; favor **first-party data** (email, pixels,
  consented UTM). A dashboard number is **input, not a command.**

## The keystone role (what depends on this)
