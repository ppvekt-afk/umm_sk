...
| Magnific (absorbed Freepik, Apr 2026) | **creative** upscaling — art/renders only | web/API | *hallucinates* detail by design; never products/documents |
...
**PNG/TIFF** → platform derivatives → `scheduling-and-queue` → **WoopSocial `POST /media` + `POST /posts`.**
